...
def obtain_pixel_metric(info_data, contours, output_directory, reference=24.25):
    pixel_metric_list=[]
    for contour in contours:
        name_pic=os.path.basename(contour[1])
        array_contour=np.array(contour[0].object_prediction_list[0].mask.segmentation[0])
        ...
        box= cv2.minAreaRect(contour_opencv)
...
def ungroup_pic(input_contours, output_path, info_file):
    n=1
    id_list=[]
